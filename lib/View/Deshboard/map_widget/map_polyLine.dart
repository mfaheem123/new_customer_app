      ...
      builder: (c) {
        ...
        return FlutterMap(
          ...
          children: [
            ...
            /// Polyline (Dynamically trims behind driver as driver moves towards drop off)
            // Obx(() {
            //   if (c.routePoints.isEmpty) return const SizedBox();
            //
            ...
            //
            //   List<LatLng> displayPoints = activeRoute;
            //
            //   if (c.driverLat.value != 0.0 && c.driverLng.value != 0.0) {
            //     LatLng driverPos = LatLng(c.driverLat.value, c.driverLng.value);
            //    // displayPoints = _getRemainingPolyline(driverPos, activeRoute);
            //   }
            //
            //   if (displayPoints.length < 2) return const SizedBox();
            //
...
//   const AnimatedCarMarker({super.key, required this.driverLocation, required this.routePoints, required this.mapController});
//
//   @override
...
// }
//
// List<LatLng> _getRemainingPolyline(LatLng driverPos, List<LatLng> polyline) {
//   if (polyline.isEmpty) return [];
//   if (polyline.length == 1) return [driverPos, polyline.first];
//
//   double minDistance = double.infinity;
//   int closestSegment = 0;
//   LatLng snappedPoint = driverPos;
//
//   for (int i = 0; i < polyline.length - 1; i++) {
//     final start = polyline[i];
//     final end = polyline[i + 1];
//     final projected = _projectPointOnSegment(driverPos, start, end);
//     final distance = _calculateDistanceSquared(driverPos, projected);
//
//     if (distance < minDistance) {
//       minDistance = distance;
//       closestSegment = i;
...
//   }
//
//   // If driver is more than ~500m away, return polyline starting from driver to polyline
//   if (minDistance > 0.00005) {
//     return [driverPos, ...polyline];
//   }
//
//   List<LatLng> remaining = [snappedPoint];
//   for (int i = closestSegment + 1; i < polyline.length; i++) {
