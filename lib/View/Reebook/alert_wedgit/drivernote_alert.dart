...
class _DriverNotesDialogState extends State<DriverNotesDialog>
  ...
  void _onNoteChanged() {
    if (mounted) {
      setState(() {
        _wordCount = _countWords(rideController.noteController.text);
      });
    }
  }
  ...
  void initState() {
    ...
    if (widget.initialNote != null && widget.initialNote!.isNotEmpty) {
      rideController.noteController.text = widget.initialNote!;
    } else if (rideController.driverNote.isNotEmpty) {
      rideController.noteController.text = rideController.driverNote;
    }
    _wordCount = _countWords(rideController.noteController.text);
    ...
    rideController.noteController.addListener(_onNoteChanged);
  }

  @override
  void dispose() {
    rideController.noteController.removeListener(_onNoteChanged);
  ...
  void _onQuickAdd(String option) {
    final current =  rideController.noteController.text.trim();
    final newText = current.isEmpty ? option : '$current. $option';
    final newWordCount = _countWords(newText);

    if (newWordCount <= _maxWords) {
      rideController.noteController.text = newText;
      rideController.noteController.selection = TextSelection.fromPosition(
        TextPosition(offset:  rideController.noteController.text.length),
      );
    }
  }

  void _onSave() {
    rideController.drivernoteText(context);
  }
